...
$execute as @s[tag=!trophy_$(section)] at @e[type=marker,tag=$(team),tag=!trophy_$(section)] run data modify storage qfe:storage SyncTeam.ItemsSync.Items set from entity @e[type=marker,tag=$(team),limit=1] data.items.$(section)
