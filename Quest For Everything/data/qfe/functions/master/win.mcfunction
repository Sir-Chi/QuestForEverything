...
execute if score oneItemPerPerson GameSetup matches 1 run tellraw @a ["",{"selector":"@e[scores={score=1171..}]"},{"text":" completed the Quest for Everything in "},{"score":{"name":"minutes","objective":"time"}},{"text":" : "},{"score":{"name":"seconds","objective":"time"}},{"text":"!"}]

playsound music.credits record @a ~ ~ ~
